...
### 認可（Authorization／オーソライゼーション）
**「誰が何をできるか」を制御する仕組み**。<br>
認証（Authentication）で「誰であるか」を確認した後、その人が特定の操作やリソースにアクセスする権限があるかを判断するプロセス。<br>

**認証 vs 認可:**<br>
- **認証（Authentication）**: 「あなたは誰ですか？」→ 本人確認<br>
- **認可（Authorization）**: 「あなたは何ができますか？」→ 権限確認<br>

**主要な概念:**<br>
- **パーミッション（Permission）**: 個別の操作権限（例: `users:read`, `posts:delete`）<br>
- **ロール（Role）**: パーミッションの詰め合わせセット（例: `Editor`, `Admin`）<br>
- **RBAC（Role-Based Access Control）**: ロールベースでアクセス制御を行う方式<br>

**現場での理解:**<br>
「権限」という言葉は、個別のパーミッションを指す場合と、ロール全体を指す場合がある。<br>
「ロールを付与する」= ユーザーに役割（複数のパーミッションのセット）を割り当てること。<br>

[認可の詳細（パーミッション、ロール、RBAC）](authorization.md)
...
**OAuth 2.0 に「認証」機能を追加した規格。**<br>
OAuth 2.0 は本来「認可（何ができるか）」のプロトコルで、「認証（誰であるか）」は保証しない。<br>
OIDC は OAuth 2.0 の上に構築され、**ユーザーの身元確認（認証）を標準化**したもの。<br>
...
|------|-----------|----------------------|
| **目的** | 認可（Authorization） | 認証（Authentication） + 認可 |
| **答える質問** | 「何ができるか？」 | 「誰であるか？」 + 「何ができるか？」 |
| **発行するトークン** | アクセストークン | アクセストークン + **IDトークン** |
| **ユーザー情報** | APIで取得（任意） | IDトークンに含まれる（標準化） |
| **用途** | API連携、リソースへのアクセス | ログイン、本人確認 |

**IDトークンとは:**<br>
OIDC の核となる概念。**JWT形式でユーザーの身元情報が含まれるトークン。**<br>

```
【IDトークンの例（Payload部分）】
{
  "iss": "https://accounts.google.com",      // 発行者（Issuer）
  "sub": "110169484474386276334",            // ユーザーID（Subject）
  "aud": "your-app-client-id",               // 対象アプリ（Audience）
  "exp": 1234567890,                         // 有効期限
  "iat": 1234564290,                         // 発行時刻
...
**アクセストークン vs IDトークン:**<br>
- **アクセストークン**: リソースにアクセスするための鍵（OAuth 2.0）<br>
- **IDトークン**: ユーザーが誰かを証明する身分証明書（OIDC）<br>

**OIDCの基本的な流れ（ソーシャルログインの例）:**<br>
```
1. ユーザーが「Googleでログイン」ボタンをクリック
2. アプリがGoogleの認証ページにリダイレクト（OAuth 2.0 の認可コードフロー）
3. ユーザーがGoogleにログインして「許可」をクリック
4. Googleが認可コードを発行してアプリにリダイレクト
5. アプリが認可コードを使ってGoogleにトークン要求
6. Googleが2つのトークンを発行:
   - IDトークン（JWT形式、ユーザー情報含む） ← OIDCの追加部分
   - アクセストークン（Google APIへのアクセス用）
7. アプリがIDトークンを検証（署名確認）
8. IDトークンからユーザー情報（メール、名前など）を取り出す
9. アプリが自社DBにユーザー登録（または既存ユーザーと紐付け）
10. アプリが独自のセッションを作成してログイン完了
```

**OIDCでできること:**<br>
- **シングルサインオン（SSO）**: 1回のログインで複数サービスにアクセス<br>
- **ソーシャルログイン**: Google、GitHub、Microsoft、LINEなどで簡単ログイン<br>
- **ユーザー情報の取得**: メール、名前、プロフィール画像などを標準化された方法で取得<br>
- **セキュアな認証**: パスワード管理を外部サービス（IdP: Identity Provider）に委譲<br>

**OIDCのメリット:**<br>
- パスワード管理不要（Google等が管理）<br>
- ユーザーの登録・ログインが簡単（既存アカウントを使える）<br>
- 多要素認証も委譲（GoogleやOktaのMFAをそのまま利用）<br>
- 標準化されているため実装しやすい（各社のライブラリが充実）<br>

**OIDCを提供する主なIdP（Identity Provider）:**<br>
- Google（Google Identity Platform）<br>
- Microsoft（Microsoft Entra ID / Azure AD）<br>
- GitHub<br>
- Auth0<br>
- Okta<br>
- LINE<br>
...
**実務での使われ方:**<br>
「Googleでログイン」「GitHubでログイン」などのボタンを見たら、それはOIDCを使っている。<br>
アプリ側はパスワードを保存せず、IDトークンから必要な情報だけを取得してユーザー管理を行う。<br>

**補足:**<br>
OAuth 2.0 単体でも「ユーザー情報API」を呼び出せばユーザー情報は取得できるが、<br>
OIDC は**標準化されたIDトークン**でユーザー情報を提供するため、実装が統一されセキュアになる。<br>
...
### ハッシュ化（Hashing／ハッシング）
データを一方向の計算で固定長の文字列（ハッシュ値）に変換する技術。<br>
**「元に戻せない」「同じ入力なら必ず同じ出力」という特徴**を持ち、パスワード保存やデータ整合性の確認に使われる。<br>

**ハッシュ化の基本特性:**<br>
- **不可逆性**: ハッシュ値から元のデータに戻すことは（現実的な時間では）不可能<br>
- **決定性**: 同じ入力からは必ず同じハッシュ値が生成される<br>
- **固定長**: 入力データのサイズに関わらず、出力は固定長（SHA-256なら256ビット＝64文字）<br>
- **雪崩効果**: 入力が1文字でも変わると、ハッシュ値は大きく変わる<br>

**具体例（SHA-256）:**<br>
```
入力: "password123"
→ SHA-256ハッシュ値: ef92b778bafe771e89245b89ecbc08a44a4e166c06659911881f383d4473e94f

入力: "password124"（最後が1文字違う）
→ SHA-256ハッシュ値: 8d5e957f297893487bd98fa830fa6413a0f34c7b5a0a5b5e3db36b3b7a1c5e42
（完全に異なるハッシュ値）
```
...
**① パスワードの安全な保存（最重要）**<br>
データベースに平文パスワードを保存するのは絶対NG。<br>
ハッシュ化して保存すれば、万が一DBが漏洩してもパスワードは守られる。<br>
```
【ユーザー登録時】
1. ユーザーがパスワード "mypassword" を入力
2. サーバーがハッシュ化: SHA-256("mypassword") → "85d8ce5..."
3. DBにハッシュ値 "85d8ce5..." を保存（平文は保存しない）
...
**② データの整合性確認**<br>
ファイルのダウンロード時に、改ざんされていないか確認する。<br>
```
公式サイトにファイルとそのSHA-256ハッシュ値を公開:
ubuntu-22.04.iso → ハッシュ値: "a2b3c4d5..."

ユーザーがダウンロード後、自分でハッシュ値を計算:
→ "a2b3c4d5..." なら改ざんなし
→ 異なれば改ざんあり、または破損
```

**③ デジタル署名**<br>
ドキュメントや通信内容のハッシュ値を秘密鍵で暗号化し、改ざん検知と本人確認を実現。<br>

**主要なハッシュアルゴリズム:**<br>
- **SHA-256**（Secure Hash Algorithm 256-bit）: 現在最も広く使われる。セキュア<br>
- **SHA-512**: SHA-256より強力だが処理が重い<br>
- **bcrypt**: パスワード専用。意図的に遅くしてブルートフォース攻撃（総当たり攻撃）を防ぐ（推奨）<br>
- **scrypt / Argon2**: bcryptより新しく、より強力なパスワードハッシュ<br>
- **MD5**: 古く、脆弱性あり。現在は非推奨<br>
- **SHA-1**: MD5より強力だが、衝突攻撃が発見され非推奨<br>

**実務での推奨:**<br>
- **パスワード保存**: bcrypt、scrypt、Argon2（計算コストが高く、攻撃耐性が強い）<br>
- **データ整合性確認**: SHA-256、SHA-512<br>
- **一般的な用途**: SHA-256（スピードとセキュリティのバランスが良い）<br>

**ソルト（Salt）による強化:**<br>
ハッシュ化だけでは「レインボーテーブル攻撃」に脆弱。<br>
レインボーテーブルとは、よくあるパスワードとそのハッシュ値を大量に事前計算した辞書。<br>
...
レインボーテーブルで検索:
→ "password" のSHA-256ハッシュと一致！
→ パスワードが "password" だとバレる
```

**ソルトの仕組み:**<br>
パスワードに**ランダムな文字列（ソルト）を追加**してからハッシュ化する。<br>
ソルトはユーザーごとに異なる値を生成し、ハッシュ値と一緒にDBに保存する。<br>

```
【登録時】
1. ユーザーがパスワード "password" を入力
2. ランダムなソルトを生成: "a3f9k2x8"
3. パスワード + ソルトをハッシュ化:
   SHA-256("password" + "a3f9k2x8") → "d4e5f6..."
4. DBに保存:
   user_id: 1
   salt: "a3f9k2x8"
   hash: "d4e5f6..."
...
【ログイン時】
1. ユーザーがパスワード "password" を入力
2. DBからsaltを取得: "a3f9k2x8"
3. 入力パスワード + ソルトをハッシュ化:
   SHA-256("password" + "a3f9k2x8") → "d4e5f6..."
4. DBのハッシュ値と照合 → 一致すれば認証成功
```

**ソルトの効果:**<br>
- 同じパスワード "password" でも、ユーザーごとに異なるハッシュ値になる<br>
- レインボーテーブルが使えなくなる（ソルト込みのテーブルを作るのは現実的でない）<br>
- 攻撃者は各ユーザーのパスワードを1つずつブルートフォース攻撃（総当たり攻撃）するしかない<br>

**bcryptの優位性:**<br>
bcryptは**ソルトが自動的に組み込まれる**ハッシュアルゴリズム。<br>
開発者がソルト処理を意識せずに安全なパスワード保存ができる。<br>
```python
# Python の bcrypt ライブラリの例
import bcrypt

# パスワードをハッシュ化（ソルトは自動生成・埋め込み）
hashed = bcrypt.hashpw(b"password123", bcrypt.gensalt())
# → b'$2b$12$KIXxLV...'（ソルト情報も含まれている）
...
| 項目 | ハッシュ化 | 暗号化 |
|------|-----------|--------|
| **方向性** | 一方向（元に戻せない） | 双方向（復号化できる） |
| **用途** | パスワード保存、整合性確認 | データの秘匿（通信、ファイル） |
| **鍵** | 不要 | 必要（秘密鍵/公開鍵） |
| **例** | SHA-256、bcrypt | AES、RSA |

**実務での注意点:**<br>
- パスワードは**必ずハッシュ化**して保存（平文保存は論外）<br>
- パスワードには**bcrypt/Argon2など専用アルゴリズム**を使う（SHA-256単体は速すぎて攻撃されやすい）<br>
- ソルトは**ユーザーごとに異なる値**を生成<br>
- 古いアルゴリズム（MD5、SHA-1）は使わない<br>

パスワード保存は最もセンシティブなセキュリティ課題の一つ。<br>
漏洩事故でユーザーの信頼を失わないよう、正しいハッシュ化とソルトの実装が必須。<br>

---

### パスワードの定期変更は必要か？

**結論：不要。むしろ逆効果になりやすい。**<br>

かつては「パスワードを3ヶ月ごとに変更しなさい」という運用が一般的だった。<br>
しかし現在は、**定期変更を義務付けることはセキュリティ上の利点がなく、逆に弱体化を招く**というのが国際的なコンセンサスになっている。<br>

**なぜ定期変更が逆効果か？**<br>
人間は面倒を嫌うため、強制変更すると以下の行動をとりやすい。<br>
- `Password1` → `Password2` → `Password3` のように末尾を変えるだけ<br>
- 数世代のパスワードをローテーションして使い回す<br>
- 結果として「予測しやすいパスワード」が量産される<br>

強制変更がなければ、1つの強いパスワードをそのまま使い続けられる。<br>

**NISTの見解（NIST SP 800-63B、2017年）:**<br>
アメリカの国立標準技術研究所（NIST）は、パスワードポリシーの指針として以下を推奨している。<br>
- 定期的なパスワード変更を**要求しない**<br>
- ただし、**漏洩が確認された場合は直ちに変更**する<br>
- 長さを重視する（最低8文字、できれば15文字以上）<br>
- 複雑さ（大文字・記号など）の強制よりも**長さ**のほうが重要<br>

**IPAの見解:**<br>
日本の情報処理推進機構（IPA）も同様の立場をとっており、パスワードの要件として以下を重視している。<br>
- **長く・複雑なパスワード**を設定する<br>
- **サービスごとに異なるパスワード**を使う（使い回しNG）<br>
- 漏洩が疑われる場合はすぐに変更する<br>
- 定期変更の義務付けは**推奨しない**<br>
...
| やること | やらなくていいこと |
|---------|----------------|
| 十分に長いパスワードを設定する（15文字以上推奨） | 定期的に変更する |
| サービスごとに異なるパスワードを使う | 複雑さのために記号を無理やり入れる |
| パスワードマネージャーを使う | 覚えられる範囲で変更し続ける |
| 漏洩時には即座に変更する | ローテーションで使い回す |
| 多要素認証（MFA）を設定する | |

**まとめ:**<br>
セキュリティの観点では、「変えない強いパスワード」は「定期的に変える弱いパスワード」より安全。<br>
本当に必要な変更タイミングは「漏洩が疑われるとき」だけ。<br>
