...
テーブルの**全データを一括削除**する専用コマンド。<br>
`DELETE FROM users;`（WHERE句なしの全件DELETE）と同じ結果になるが、内部の仕組みが根本的に異なる。<br>
...
| 項目 | DELETE（全件） | TRUNCATE |
|------|--------------|----------|
| **処理方式** | 1行ずつ削除 | テーブルを丸ごとリセット |
| **速度** | 遅い（行数に比例） | 速い（行数に関係なく一瞬） |
| **WHERE句** | 使える（条件指定可能） | 使えない（全件のみ） |
| **ロールバック** | できる（トランザクション内で） | DBによって異なる（MySQLは不可） |
| **AUTO_INCREMENT** | リセットされない（続き番号） | **リセットされる（1から再開）** |
| **トリガー発火** | 発火する | 発火しない |
| **外部キー制約** | 制約チェックあり | 制約があるとエラーになる場合も |

### AUTO_INCREMENT のリセットに注意

実務でよく引っかかるポイント。<br>

```sql
-- 現在 id=100 まで登録済みのテーブルを全件削除した場合
...
**TRUNCATE を使うべき場面：**<br>
- 開発・テスト環境でテストデータを一括削除したい<br>
- ログテーブルや一時テーブルを定期的にリセットしたい<br>
- データ量が多く、削除に時間をかけたくない<br>

**DELETE を使うべき場面：**<br>
- 条件を指定して一部だけ削除したい<br>
- ロールバックできる安全な削除が必要<br>
- 本番環境でのデータ削除（TRUNCATEはロールバック不可のDBが多い）<br>
- 外部キー制約で参照されているテーブルを削除したい<br>

**⚠️ 実務での注意：**<br>
TRUNCATEは一瞬で全データが消え、MySQLでは**ロールバックできない**。<br>
本番環境で誤って実行すると取り返しがつかないため、実行前に必ず対象テーブルとバックアップを確認すること。<br>
