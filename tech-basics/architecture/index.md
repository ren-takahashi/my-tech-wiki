...
**別名:** 水平スケーリング（Horizontal Scaling）<br>
...
**別名:** 垂直スケーリング（Vertical Scaling）<br>
...
### CI/CD（シーアイ・シーディー）
**CI（Continuous Integration / 継続的インテグレーション）** と **CD（Continuous Delivery/Deployment / 継続的デリバリー/デプロイメント）** の略。<br>
コードの変更を自動でテスト・ビルド・デプロイする仕組み。<br>

**CI（継続的インテグレーション）:**<br>
開発者がコードをリポジトリにプッシュするたびに、自動でテストやビルドを実行。<br>
問題があればすぐに検知できる。<br>
...
**CD（継続的デリバリー/デプロイメント）:**<br>
テストに合格したコードを、自動でステージング環境や本番環境にデプロイする。<br>

**Continuous DeliveryとDeploymentの違い:**<br>
- **Continuous Delivery（デリバリー）**: 自動で「デプロイ可能な状態」にする。本番リリースは手動承認<br>
- **Continuous Deployment（デプロイメント）**: 本番環境まで完全自動でデプロイ<br>

**CI/CDのメリット:**<br>
- バグの早期発見（コードを書いたらすぐテスト）<br>
- 手作業によるミス削減（自動化により人為的ミスを防ぐ）<br>
- リリース頻度の向上（いつでもリリース可能な状態を維持）<br>
- 開発スピードの向上（手動作業を待つ時間が減る）<br>

**CI/CDツールの例:**<br>
- **GitHub Actions**: GitHubに統合されたCI/CDツール<br>
- **GitLab CI/CD**: GitLabに統合<br>
- **Jenkins**: オープンソースの老舗CI/CDツール<br>
- **Azure DevOps**: Microsoft提供のDevOpsプラットフォーム<br>
- **CircleCI、Travis CI**: クラウドベースのCI/CDサービス<br>
...
最終確認
↓
CD: 本番環境にデプロイ（手動承認または自動）
```

CI/CDはDevOpsの中心的な実践手法で、現代の開発では標準的な仕組みとなっている。<br>
...
例えば、フロントはWebサーバー、バックエンドは複数のマイクロサービス（ユーザー管理、商品管理、注文管理など）に分かれていて、それぞれ実行環境が独立しているイメージ。<br>
利点：独立したスケーリング、技術スタックの自由度、障害の局所化<br>
課題：サービス間通信の複雑さ、分散トランザクション、運用の難しさ<br>

**モノリシックアーキテクチャ（モノリス型）:**<br>
すべての機能が1つのアプリケーションにまとまった構造。<br>
フロントエンド、バックエンド、データベースが一体となっているイメージ。<br>
...
ここでの依存とは、**あるクラスが、他のクラスの「具体的な実装詳細」を知らないと成立しない状態**を指す。<br>

依存が強い例:
- UseCaseが `Cake\Http\ServerRequest` を直接受け取る(フレームワークのクラスを知っている。例はCakePHP。) 
- UseCaseがORMのTableを直接操作する。（interfaceを介さず、具体的な実装を知っている）
- UseCaseがHTTPレスポンス生成を知っている。

依存が弱い例:  
- UseCaseはDTO（データクラスとして定義したrequest/responseクラス）を受け取るだけ
- DBアクセスはinterface経由
- 外部APIは抽象化されている(Service層で実装したり、interfaceを定義している)

小規模プロジェクトでは、クリーンアーキテクチャの全ての層を厳密に分ける必要はない。<br>
大規模プロジェクトや長期運用が見込まれるプロジェクトでは、クリーンアーキテクチャの原則を守ることで、変更に強い設計になる。<br>
どこまでやるかはプロジェクトの規模やチームの方針次第だが、以下はこのアーキテクチャにかかわらず常に意識しても良いポイント。<br>
- 依存は外→内に向ける
- 業務ロジックを外側の技術から守る
- 責務を分ける
- テストしやすい構造にする
...
フロントエンドのUI設計でよく使われるパターン。<br>
Model、View、ViewModelの3層構造。<br>
ViewModelがViewの状態を管理し、双方向データバインディングを提供する。<br>
WPF、Angular、Vue.jsなどが採用されることが多い。<br>
Reactは少し違い、MVVMにもできる”が、標準的ではない。<br>
利点：UIロジックの分離、テストしやすい、データバインディングによる開発効率向上<br>

---
### フロントエンドレンダリング戦略

Webアプリケーションで、HTMLをどこで生成（レンダリング）するかの設計方針。<br>
主に**CSR（クライアントサイドレンダリング）**と**SSR（サーバーサイドレンダリング）**がある。<br>

**CSR（Client-Side Rendering / クライアントサイドレンダリング）:**<br>
ブラウザ（クライアント側）でJavaScriptを使ってHTMLを生成する方式。<br>
サーバーは最小限のHTMLとJavaScriptファイルを返すだけ。<br>

**仕組み:**<br>
```
1. サーバーから空のHTMLとJavaScriptファイルをダウンロード
2. ブラウザでJavaScriptを実行
3. JavaScriptがAPIからデータを取得
4. 取得したデータを元にHTMLを動的生成
5. 画面に表示
```

**メリット:**<br>
✅ サーバー負荷が軽い（HTMLを生成する処理がサーバー側にない）<br>
✅ 画面遷移が高速（一部だけを更新できる、SPA体験）<br>
✅ インタラクティブなUIが作りやすい<br>

**デメリット:**<br>
❌ 初回表示が遅い（JavaScriptダウンロード + API呼び出しが必要）<br>
❌ SEOに不利（検索エンジンがJavaScript実行前のHTMLを見る場合がある）<br>
❌ JavaScriptオフだと動かない<br>

**代表的なフレームワーク:**<br>
React（Create React App）、Vue.js、Angular<br>

---

**SSR（Server-Side Rendering / サーバーサイドレンダリング）:**<br>
サーバー側で完成したHTMLを生成してからブラウザに返す方式。<br>
ブラウザは受け取ったHTMLをそのまま表示できる。<br>

**仕組み:**<br>
```
1. ブラウザがサーバーにリクエスト
2. サーバー側でデータ取得 + HTML生成
3. 完成したHTMLをブラウザに返す
4. ブラウザがHTMLを即座に表示
5. （その後、JavaScriptで動的機能を有効化）
```

**メリット:**<br>
✅ 初回表示が速い（完成したHTMLが届くのでそのまま表示できる）<br>
✅ SEOに有利（検索エンジンが完全なHTMLを読める）<br>
✅ JavaScriptオフでも基本的なコンテンツは表示される<br>

**デメリット:**<br>
❌ サーバー負荷が高い（毎回HTMLを生成する処理が走る）<br>
❌ 画面遷移で毎回フルリロードが発生しやすい（従来型の場合）<br>
❌ サーバーのレスポンス待ちが発生する<br>

**代表的なフレームワーク:**<br>
Next.js（React）、Nuxt.js（Vue.js）、従来型のPHP/Ruby on Rails<br>

---

**SSG（Static Site Generation / 静的サイト生成）:**<br>
ビルド時に全ページのHTMLを事前生成しておく方式。SSRの一種。<br>
サーバーは生成済みのHTMLを返すだけなので超高速。<br>

**使い所:**<br>
- ブログ、ドキュメントサイトなど、頻繁に更新されないコンテンツ<br>
- 全ページ数が予測可能で少ない場合<br>

**代表的なフレームワーク:**<br>
Gatsby、Next.js（SSGモード）、Astro<br>
...
| 要件 | 推奨方式 | 理由 |
|------|---------|------|
| **コーポレートサイト、ブログ** | SSR/SSG | SEO重視、初回表示速度重視 |
| **ECサイト商品ページ** | SSR | SEO必須、初回表示が重要 |
| **管理画面、ダッシュボード** | CSR | SEO不要、インタラクション重視 |
| **SNS（タイムライン）** | CSR | リアルタイム更新が多い |
| **ニュースサイト** | SSR/SSG | SEO重要、初回表示速度重視 |
| **SaaS（Slack等の業務ツール）** | CSR | ログイン後の画面、SEO不要 |

**実務での判断ポイント:**<br>
- **SEOが必要か？** → YES なら SSR/SSG<br>
- **初回表示速度が重要か？** → YES なら SSR/SSG<br>
- **リアルタイム性・インタラクションが中心か？** → YES なら CSR<br>
- **サーバー負荷を抑えたいか？** → YES なら CSR または SSG<br>

**ハイブリッド構成（Next.jsなど）:**<br>
現代のフレームワークは、ページごとにCSR/SSR/SSGを使い分けられる。<br>
- トップページ・商品ページ → SSR（SEO重視）<br>
- マイページ・管理画面 → CSR（インタラクション重視）<br>
- 会社概要・利用規約 → SSG（静的コンテンツ）<br>

---
### API設計
アプリケーション間やシステム間でデータをやり取りする仕組みの設計。<br>

**API（Application Programming Interface）とは:**<br>
ソフトウェア同士が通信するための「窓口」や「約束事」。<br>
Webサービスでは、サーバーとクライアント（ブラウザやスマホアプリ）がデータをやり取りする仕組みを指すことが多い。<br>

**主な通信方式:**<br>

**▼ RESTful API（Representational State Transfer）:**<br>
HTTPメソッド（GET、POST、PUT、DELETEなど）を使った**リクエスト・レスポンス型**の通信方式。<br>
現在最も広く使われているAPI設計スタイル。<br>

**特徴:**<br>
- クライアントがリクエストを送り、サーバーがレスポンスを返す（一方向）<br>
- リソース（データ）をURLで表現（例: `/api/users/123`）<br>
- HTTPメソッドで操作を表現（GET=取得、POST=作成、PUT=更新、DELETE=削除）<br>
- ステートレス（サーバーはクライアントの状態を保持しない）<br>

**使用例:**<br>
- ユーザー情報の取得（`GET /api/users/123`）<br>
- 商品の登録（`POST /api/products`）<br>
- 記事の更新（`PUT /api/articles/456`）<br>
- データの削除（`DELETE /api/comments/789`）<br>

**▼ WebSocket API:**<br>
サーバーとクライアント間で**双方向リアルタイム通信**を実現するプロトコル。<br>
一度接続を確立すると、お互いが自由にデータを送受信できる。<br>

**特徴:**<br>
- 持続的な接続を維持（接続したまま）<br>
- サーバーからクライアントへ自発的にデータ送信可能（プッシュ通信）<br>
- リアルタイム性が求められる用途に最適<br>
- HTTPより通信のオーバーヘッドが少ない<br>

**使用例:**<br>
- チャットアプリ（メッセージのリアルタイム送受信）<br>
- リアルタイム通知（新着情報をサーバーからプッシュ）<br>
- オンラインゲーム（プレイヤーの動きを即座に同期）<br>
- 株価・為替表示（価格変動をリアルタイム更新）<br>
- 共同編集ツール（複数人で同時編集）<br>

**RESTful API と WebSocket の使い分け:**<br>

| 項目 | RESTful API | WebSocket |
|------|-------------|-----------|
| **通信方向** | 一方向（リクエスト→レスポンス） | 双方向（お互いに送信可能） |
| **接続** | 毎回接続・切断 | 持続的な接続 |
| **用途** | データの取得・登録・更新・削除 | リアルタイム通信 |
| **サーバーからの送信** | 不可（クライアントの要求が必要） | 可能（自発的にプッシュ） |

実務では、通常のデータ操作にRESTful API、リアルタイム機能にWebSocketというように**両方を併用**することが多い。<br>
...
オブジェクト指向設計の5つの基本原則。<br>
  ...
  - **L**: Liskov Substitution（リスコフの置換原則）- 派生クラス(子クラス)は基底クラス(親クラス)と置き換え可能<br>
